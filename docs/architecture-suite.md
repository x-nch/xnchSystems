...
flowchart TB
    ...
    subgraph NodeA["Node A — 192.168.50.1 (control + memory)"]
        subgraph DockerA["Docker Compose"]
            ...
            LF["Langfuse :3000"]
            SXR["SearXNG :8888<br/>loopback only"]
        end

        subgraph SystemdA["systemd"]
            XNCH["xnch :8001<br/>FastAPI gateway"]
            MCPB["xnch_mcp bridge<br/>McpBridgePool"]
            CONS["consolidation.timer<br/>POST /admin/consolidate"]
        end

        subgraph BridgeChildren["MCP stdio children"]
            CRG["code-review-graph<br/>uvx serve · crg_ · T0"]
            AM["agentmemory<br/>npx @agentmemory/mcp · am_ · T1"]
            DOC["docs-test<br/>python -m docs_test_mcp · doc_ · T0"]
            C7["context7 · c7_ · T0<br/>(disabled)"]
        end

        AMSVC["agentmemory.service :3111"]
    ...
    XNCH --> SQLITE
    XNCH --> AUDIT
    XNCH -->|"spawns stdio children"| MCPB
    MCPB --> CRG
    MCPB --> AM
    MCPB --> DOC
    MCPB -.->|"disabled"| C7
    AM --> AMSVC
    XNCH -->|"xnch_web_search"| SXR
    XNCH -->|"POST /v1/chat/completions"| LLM
    XNCH -->|"POST /nexi/chat + MCP tool loop"| LLM
...
flowchart LR
    ...
    subgraph NodeA["Node A — gate7 / i7 / 192.168.50.1"]
        ...
        DC --> searxng["searxng :8888<br/>127.0.0.1 only"]
...
flowchart TB
    ...
    subgraph Curated["Curated agent memory (parallel, not synced)"]
        AM["agentmemory :3111<br/>am_* MCP bridge<br/>lessons · actions · facts"]
    end
    ...
    AM -.->|"explicit am_* tools only"| NEXI_CHAT["/nexi/chat tool loop"]
...
> **Dual memory:** L2 pgvector is auto-injected in every `/nexi/chat` turn.
