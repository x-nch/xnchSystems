...
- `xnch/` — git submodule → github.com/x-nch/xnch (control plane API: REST routes, auth, memory)
- `nexi/` — git submodule → github.com/x-nch/nexi (execution engine: FastAPI, decision/policy system)
...
cd nexi && pytest tests                  # Run only nexi tests
cd xnch && pytest tests                  # Run only xnch tests
cd nexi && pytest tests/test_evaluator.py  # Run single test file
```
